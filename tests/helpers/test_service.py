...
async def test_call_no_context_target_all(
    ...
    await service.entity_service_call(
        ...
        [Mock(entities=mock_entities)],
        Mock(),
        ha.ServiceCall("test_domain", "test_service", {"entity_id": ENTITY_MATCH_ALL}),
    )
...
async def test_call_with_match_all(hass, mock_service_platform_call, mock_entities):
    """Check we only target allowed entities if targetting all."""
    await service.entity_service_call(
        hass,
        [Mock(entities=mock_entities)],
...
async def test_call_with_omit_entity_id(
    ...
    """Check error is raised if no entity id passed in service."""
